...
impl AgentConversationsModel {
    ...
    fn task_creator_name(task: &AmbientAgentTask, app: &AppContext) -> Option<String> {
    ...
    fn task_creator_uid(task: &AmbientAgentTask) -> Option<String> {
    ...
    pub fn get_all_creators(&self, app: &AppContext) -> Vec<(String, String)> {
        let mut creators: Vec<(String, String)> = self
            ...
            .values()
            .filter_map(|task| {
                let name = Self::task_creator_name(task, app)?;
                let uid = Self::task_creator_uid(task)?;
                Some((name, uid))
            })
            .collect();
